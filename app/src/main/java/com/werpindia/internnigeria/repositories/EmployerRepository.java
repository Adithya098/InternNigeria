...
public class EmployerRepository {
    ...
    public Single<Employer> getCurrentEmployer() {
        return Single.create((SingleOnSubscribe<Employer>) emitter -> {
            if (!emitter.isDisposed()) {
                String email = auth.getCurrentUser().getEmail();
                db.collection(EMPLOYER_COLLECTION_NAME).whereEqualTo("email", email).get().addOnCompleteListener(task ->
                {
                    if (task.isSuccessful()) {
                        QuerySnapshot snapshot = task.getResult();
                        if (snapshot != null && !snapshot.getDocuments().isEmpty())
                            emitter.onSuccess(snapshot.getDocuments().get(0).toObject(Employer.class));
    ...
    public Single<Employer> getEmployer(String email) {
        return Single.create((SingleOnSubscribe<Employer>) emitter ->
        {
            if (!emitter.isDisposed())
                db.collection(EMPLOYER_COLLECTION_NAME).whereEqualTo("email", email).get().addOnCompleteListener(task -> {
